...
import { womenCrausel as images } from '../Utils/crauselData';
...
const Women = () => {
  ...
  useEffect(() => {
      setisLoaded(true);
      let newUrl =`${url}/pagenation?filterBy=gender&filterName=female&pageNo=${page}&limit=10`
      getData(newUrl)
  }, [page])


  const getData = async (newUrl) => {
      // eg = filterBy=gender,filter=female,
      try {
          setisLoaded(true)
  ...
  const handleSorting = (e)=>{
      ...
      switch(e.target.value){
          case "inc" :{
              let newUrl =`${url}/pagenation?filterBy=gender&filterName=female&pageNo=${page}&limit=10&sortBy=price&sortOrder=1`
              getData(newUrl)
          }
          case "desc" :{
              let newUrl =`${url}/pagenation?filterBy=gender&filterName=female&pageNo=${page}&limit=10&sortBy=price&sortOrder=-1`
